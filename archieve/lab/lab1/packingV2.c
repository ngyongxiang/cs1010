...
int main(void){
	// Declare variables
	int tray_len,tray_width,slab_len,slab_width,max_slabs;
    ...
    //Calculate result
    max_slabs=compute_max_slabs(tray_len,tray_width,slab_len,slab_width);
...
int compute_max_slabs(int tray_len,int tray_width,int slab_len,int slab_width){
	// Declare variables
    int h_fit,v_fit,row,col,tray_len_left,tray_width_left;
    ...
    // Now we check whether we can fit in some more slabs vertically.
    tray_len_left=tray_len; // Available tray length left after filling slabs up horizontally
    tray_width_left=tray_width%slab_len; //Available tray width
    
    
    if (tray_width_left>=slab_width){
        // if we can fit in, we do some calculation and add the number into h_fit
    	row=tray_width_left/slab_width;
    	col=tray_len_left/slab_len;
    ...
    // Now we check whether we can fit in some more slabs horizontally.
    tray_len_left=tray_len%slab_width;
    tray_width_left=tray_width;


     if (tray_width_left>=slab_len){
        // if we can fit in, we do some calculation and add the number into v_fit
    	row=tray_len_left/slab_width;
    	col=tray_width_left/slab_len;
